...
- (instancetype)initWithFrame:(CGRect)frame viewController:(BaseViewController *)viewController layout:(KKClassiflcationLayout *)layout clickBlock:(ClickBlock)block
{
    if (self = [super initWithFrame:frame]) {
        ...
        for (BaseViewController *vc in self.layout.viewControllers) {
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BaseViewController *subVC = self.layout.viewControllers[indexPath.row];
    subVC.navBar.hidden = YES;
...
- (BaseCollectionView *)collection
{
    if (!_collection) {
        ...
        _collection = [[BaseCollectionView alloc] initWithFrame:CGRectMake(0, self.layout.sliderHeight, WIDTH_SCREEN, self.frame.size.height - self.layout.sliderHeight) collectionViewLayout:layout];
